...
type NewsAggPageCom1 struct {
	Title string
	News  map[string]NewsMapCom1
}

//a basic index page , this is not a template
func index_handlerCom1(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "<h1 align=\"center\">Templating</h1>")
}

func newsRoutine(c chan NewsCom1, Location string) {
	defer wg4.Done()
	var n NewsCom1
	resp, _ := http.Get(Location)
	bytes := washpostXMLCom1
	xml.Unmarshal(bytes, &n)
	resp.Body.Close()
	c <- n
}

func newsAggHandlerCom1(w http.ResponseWriter, r *http.Request) {

	var s SitemapIndexCom1
	var n NewsCom1
	news_map := make(map[string]NewsMapCom1)
	bytes := washpostXMLCom1
	...
	for _, Location := range s.Locations {
		...
		for elem := range queue {
			for idx, _ := range elem.Keywords {
				news_map[elem.Titles[idx]] = NewsMapCom1{elem.Keywords[idx], elem.Locations[idx]}
			}
		}

		for idx, _ := range n.Titles {
			news_map[n.Titles[idx]] = NewsMapCom1{n.Keywords[idx], n.Locations[idx]}

		}
	}

	p := NewsAggPageCom1{Title: "Washington Post News Aggregator", News: news_map}
...
func main() {
	...
	http.HandleFunc("/", index_handlerCom1)
	http.HandleFunc("/agg", newsAggHandlerCom1)
